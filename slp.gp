/*
	...
	(out[p[i]])_{i=1,...,#p}. 
*/
evalslp(grp_data, slpdata, {h=0}, {grp_vecmul=0}, {reverse=0}, {retout=0})={;
	...
	my(slp, pointers, n);
	slp=slpdata[1];
	if(h, n=#h; grp_gens=vector(n, u, grp_gens[h[u]]););
	n=#grp_gens;
	if(grp_vecmul,
		if(#slpdata==1,
			return(grp_vecmul(grp, grp_gens, slptovec(slpdata[1],n,h)));
		);
	);
	pointers=slpdata[2];
...
algevalslp(A, elts, slpdata, {reverse=0}, {retout=0})={
	my(slp, pointers);
	slp=slpdata[1];
	if(#slpdata==1,
		n=#elts;
